...
class ItemsList extends React.Component {
  ...
  apiCallForFetchCharacters = (response, apicall, name = null) => {
    ...
    this.setState({ itemsToDisplay: response.data.results });
    this.setState({ totalPages: response.data.info.pages})
  ...
  fetchPage = (e) => {
    ...
    Api.getCharactersByPage({ page: e }).then((response) => {
      this.apiCallForFetchCharacters(response);
    });
  }
  ...
  render() {
    ...
    if (!this.state.isLoading && !this.state.fetchDataError) {
      list = this.state.itemsToDisplay.map((character) => (
        <CardItem
          name={character.name}
          id={character.id}
          key={character.id}
            ...
            <>
              <Container fluid className="mx-2 px-lg-5 mt-4">
                <Row className="mt-xs-3 mt-lg-5">
                  <Col
                    xs={this.state.areResultsFiltered ? 8 : 12}
                    md={this.state.areResultsFiltered ? 8 : 12}
                  >
                    <SearchBar filter={this.doFiltering} />
                  </Col>
                  <Col
                    xs={this.state.areResultsFiltered ? 4 : 12}
                    md={this.state.areResultsFiltered ? 4 : 12}
                    className="d-flex justify-content-end pr-3"
                  >
                    {this.state.areResultsFiltered && (
                      <ErrorButton
                        text="Quitar filtro"
                        action={this.initialFetch}
                      />
                    )}
                  </Col>
                </Row>
                <Row className="mt-5 d-flex justify-content-center">
                  <CardDeck className="justify-content-center">{list}</CardDeck>
                </Row>
                <BaseErrorModal
                  show={this.state.fetchDataError}
                  handleClose={this.modalClose}
                  errorMessage="Ha ocurrido un error al intentar obtener los artículos, intenta de nuevo más tarde"
                />

                
                <Paginator pageCount={this.state.totalPages} newPage={(e)=>this.fetchPage(e)}></Paginator>
              </Container>
            </>
          )
        )}
